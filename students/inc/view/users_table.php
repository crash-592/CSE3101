            ...
            <tr class="user-row">
                ...
                <td>
                    <?php if ($_SESSION['user_id'] == $user->get_id() || $_SESSION['user_role'] == 'superuser'): ?>
                        <a href="/students/change-password?id=<?= $user->get_id() ?>" class="btn btn-sm btn-warning">Change Password</a>
                    <?php endif; ?>

                    <?php if ($_SESSION['user_role'] == 'superuser' && $_SESSION['user_id'] != $user->get_id()): ?>
                        <form method="post" action="/students/delete-user" style="display:inline;">
...
<td>
    <?php if ($_SESSION['user_id'] == $user->get_id() || $_SESSION['user_role'] == 'superuser'): ?>
        <a href="/students/change-password?id=<?= $user->get_id() ?>" 
    <?php endif; ?>
